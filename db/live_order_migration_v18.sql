-- ============================================================================
-- v18: 샘플 상품은 재고 0이어도 품절로 표시하지 않음.
--   get_live_shop 옵션 available 계산을 sum(qty)>0 → (sum(qty)>0 OR 샘플)로 변경.
--   샘플(stock.sample=true)은 재고가 없어도 주문 가능(입금확인 시 자동 미송 처리됨).
--   그 외 로직/반환은 v15와 동일. 재실행 안전(create or replace).
-- ============================================================================
