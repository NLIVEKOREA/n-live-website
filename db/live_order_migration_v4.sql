-- ============================================================================
-- N-LIVE 라이브 v4 (멱등) — v3 포함 + 개별상품(미재고) 품번 + 방송유형(원정/사무실)
--  · session_products.manual_code : 재고 미연동 개별상품의 품목번호(번호표로 표시)
--  · live_sessions.broadcast_type  : '원정' | '사무실' (구분용)
--  · get_live_shop / create_live_order : 개별상품 code = manual_code 반영
-- 실행: SQL Editor 붙여넣고 Run. (v3 위에 덮어씀)
-- ============================================================================

alter table public.session_products add column if not exists manual_code text;
alter table public.live_sessions   add column if not exists broadcast_type text;
...
-- 주문 생성 RPC v4 (개별상품 code = manual_code)
